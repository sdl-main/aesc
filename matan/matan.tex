...
\begin{minipage}{0.6\linewidth}
  ...
  Построим диаметр $PQ$. $\angle AOQ = 0.5\angle AZQ$ будет постоянным, значит точка $Q$ будет двигаться
  по прямой $OQ$.
  $\angle POQ$ постоянный и равен $90^o$. Т.к. точка $Q$ движется по прямой $OQ$, то точка $P$ будет двигаться по
  прямой $OP$, перпендикулярной $OQ$.
  Замечаем, что $OP \perp OQ, PQ = const$, а значит мы свели задачу к предыдущей.
\end{minipage}}
